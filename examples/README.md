...
A complete, runnable example showing how to:
...
- Deploy router-dependent compute resources by default (router, instance, data volume, floating IP, and security group attachment)
- Open SSH, HTTPS, and ICMP ping access on the example security group
- Attach an extra data volume to the instance, separate from its root disk
...
The default creates billable router, instance, data volume, and floating IP resources. The HTTPS bootstrap assumes an apt-based Ubuntu/Debian image and can be disabled with `-var='enable_https_bootstrap=false'`.
